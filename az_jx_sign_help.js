...
$.appId = 10028;
// 指定开助力码个数,未设置则使用默认值
var azmain = process.env.azmain ?? "3";
let helpcodes = [];
let helpstart = 0;
// 失败循环最大次数后切换下一个助力码
let failend = 3;
...
const authorCode = authorCodes[randomNum(0, authorCodes.length - 1)]
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (i < azmain) {
        if (i == 0) {
            ...
            if (helpcodes[0] == '助力已满或签到失败' || helpcodes[0] == '获取失败') {
                ...
                assistFriendRes = await helpSignhb(helpcodes[0]);
            }
        }
        await $.wait(2000);
    ...
    if (i == azmain) console.log(`助力码获取情况 --- ${helpcodes}`);
    if (helpstart == azmain) {
        console.log(`全部助力任务完成`);
        break;
    }
    if (i > azmain) {
        ...
        await $.wait(1000); //助力账号间隔时间
        assistFriendOne = await helpSignhb(helpcodes[helpstart]);
        if (assistFriendOne && assistFriendOne.ret == 0) {
            console.log(`${i} 助力 ${helpstart} ${helpcodes[helpstart]}...`);
        } else {
            await $.wait(1000); //助力失败增加间隔时间
        }
        // 每10次助力后查询被助力号是否已满
        if (i > 15 && isPositiveIntegerTimes(i,10)) {
            cookie = cookiesArr[helpstart];
            await $.wait(3000);
            checkFriendRes = await helpSignhb();
            if (checkFriendRes && checkFriendRes.ret == 0) {
                leftNeed = checkFriendRes.sharetask.status
                if (leftNeed == 2) {
                    console.log(`${failstart} 轮后 ${helpstart} ${helpcodes[helpstart]} 助力已满`);
                    helpstart++;
                    failstart = 0
                }
                if (leftNeed == 1) {
                    failstart++
                    console.log(`${failstart} 轮后 ${helpstart} ${helpcodes[helpstart]} 仍需助力`);
                    if (failstart > failend) {
                        console.log(`${failstart} 轮后 ${helpstart} ${helpcodes[helpstart]} 暂时无法满助力,跳过`);
                        helpstart++;
                        failstart = 0
                    }
                }
    ...
    $.get(taskUrl("signhb/query", `type=1&signhb_source=1000&smp=${smp}&ispp=0&tk=`), async (err, resp, data) => {
      try {
        if (err) {
          ...
          datapre = data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1];
          if (datapre.indexOf("area_type") != -1){
            data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]);
          } else {
              data = JSON.stringify("其它错误")
          }
        }
      } catch (e) {
        $.logErr(e, resp);
...
function getUA() {
    ...
    return `jdpingou;iPhone;${$.appVersion};${$.osVersion};${$.UUID};M/5.0;${network};ADID/;model/${$.mobile};addressid/;appBuild/${$.build};jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS ${$.osVersion.replace(/\./g, "_")} like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1;`
}
